        ...
        <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-lg`}>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${depotGain >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                €{depotGain.toLocaleString('de-DE', {minimumFractionDigits: 2, maximumFractionDigits: 2})} 
                ({depotInvested > 0 ? ((depotGain / depotInvested) * 100).toFixed(2) : '0.00'}%)
              </span>
            </div>
          </div>
        </div>
